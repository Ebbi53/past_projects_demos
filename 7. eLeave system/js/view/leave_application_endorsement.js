...
// Leave balance
define(['jquery', 'underscore', 'backbone', 'moduleConfig', 'text!template/leave_application_endorsement.html', 'collection/leave_application_endorsement', 'model/leave_endorse', 'model/endorsement_leave_reject', 'model/team_leave_history', 'model/user_status', 'collection/public_holidays', 'datepicker', 'moment'], function ($, _, Backbone, moduleConfig, tmpl, LeaveApplicationEndorsementCollection, LeaveEndorse, LeaveReject, TeamLeaveSchedule, statusModel, PublicHolidays, datepicker, moment) {
  var LeaveApplicationEndorsementView = Backbone.View.extend({
    tagName: 'div',
    className: 'leaveApplicationEndorsementView container fadeIn',
    ...
    initialize: function initialize(options) {
      ...
      this.collection = this.collection || new LeaveApplicationEndorsementCollection(options.collectionJSON || {});
      this.isOneTime = options.isOneTime || false;
      this.zgEndorserKey = options.zgEndorserKey || '';
    ...
    drawDayDetail: function drawDayDetail(event, data) {
      ...
      } else if (!$element.hasClass('holiday')) {
        _.defer(function () {
          var $optionRow = $('<tr>'),
            $batchSelectionRow = $('<tr>'); //$pmRow = $('<tr>');
    ...
    approveLeaveApplication: function approveLeaveApplication(event) {
      ...
      var leaveApproveObj = new LeaveEndorse();
      $.confirm({
        title: 'Confirm',
        content: 'Endorse this leave?',
        type: 'blue',
        animation: 'none',
        buttons: {
          only: {
            ...
            action: function action() {
              ...
              if (!that.isOneTime) {
                ...
                data = {
                  zgEndorserKey: that.zgEndorserKey,
                  zgTrxToken: trxToken
                };
                that.collection.reset();
              }
              ...
              leaveApproveObj.fetch({
                ...
                success: function success() {
                  ...
                  Backbone.trigger("global:success", {
                    msg: 'Leave application has been endorsed.'
                  });
    ...
    rejectLeaveApplication: function rejectLeaveApplication(event) {
      ...
      if (!this.isOneTime) {
        ...
        data = {
          zgEndorserKey: this.zgEndorserKey,
          zgTrxToken: trxToken,
          zgRejectReason: comment
        };
      ...
      leaveRejectObj.fetch({
        ...
        success: function success() {
          that.prepareDataAndRender();
          Backbone.trigger("global:success", {
            msg: 'Leave endorsement has been rejected.'
          });
          that.updateUserStatus();
        },
        error: function error(collection, response, options) {
  ...
  return LeaveApplicationEndorsementView;
});